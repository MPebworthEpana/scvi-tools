#!/usr/bin/env bash
# Run MultiVI CSR vs Zarr dataloader benchmark in Ubuntu WSL with the RAPIDS conda env.
#
# Usage (from Windows PowerShell or WSL):
#   wsl bash scripts/run_benchmark_csr_vs_zarr_wsl.sh
#   wsl bash scripts/run_benchmark_csr_vs_zarr_wsl.sh --limit-train-batches 200
#
...
cd "${REPO_ROOT}"
exec python scripts/benchmark_multivi_csr_vs_zarr.py \
  --pin-memory \
  --output-json scripts/benchmark_multivi_csr_vs_zarr_wsl_results.json \
  "$@"
