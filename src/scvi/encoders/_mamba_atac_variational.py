"""Variational encoder facade for tokenized Mamba3 ATAC."""
...
class MambaAtacVariationalEncoder(nn.Module):
    """Encode padded ATAC peak tokens into a variational latent."""
    ...
    def __init__(
        ...
        self.n_cat_list = list(n_cat_list or [])
        if n_batch > 0:
            self.n_cat_list = [n_batch] + self.n_cat_list

        self.register_buffer("coord_table", coord_table.long())
        self.embedding = AtacPeakEmbedding(d_model, binarize_values=binarize)
        mamba3_kwargs = mamba3_kwargs or {}
        self.backbone = BidirectionalMamba3Encoder(
            d_model=d_model,
            n_layers=n_layers,
            n_heads=n_heads,
    ...
    def forward(
        ...
        *cat_list: torch.Tensor,
        ...
        pooled = self.backbone(tokens, token_mask, chrom=chrom)
