...
class MultiheadAttentionBlock(nn.Module):
    ...
    def _attend(
        ...
        if logit_bias is not None:
            scores = scores + logit_bias[:, None, None, :]
        if key_mask is not None:
            scores = scores.masked_fill(~key_mask[:, None, None, :], float("-inf"))
        attn = torch.softmax(scores, dim=-1)
        attn = torch.nan_to_num(attn)
        attn = self.dropout(attn)
        ctx = (attn @ v).transpose(1, 2).reshape(b, n_q, self.d_model)
...
class InducedSetAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, key_mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        h = self.mab1(inducing, x, key_mask=key_mask)
...
class SetTransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        pooled = self.pool(h, key_mask=key_mask, inclusion_prob=inclusion_prob)
        if self.cardinality is not None and key_mask is not None and n_total is not None:
            n_obs = key_mask.sum(dim=1, keepdim=True).float()
