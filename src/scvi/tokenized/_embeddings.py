...
class AtacPeakEmbedding(nn.Module):
    def __init__(
        ...
        self.value_as_bias = value_as_bias
        if value_as_bias and binarize_values:
            self.value_bias = nn.Parameter(torch.zeros(d))
        else:
            self.value_emb = ValueMLP(d)

    def build_static_cache(self, coord_table: torch.Tensor) -> None:
        """Precompute per-peak static embedding components from ``coord_table``."""
        table = coord_table.long()
        chrom = table[:, 0]
        start = table[:, 1]
        end = table[:, 2]
        length = (end - start).clamp(min=0)
        self.register_buffer("_static_chrom", chrom)
        self.register_buffer("_static_pos_sin", sinusoidal_encoding(start, self.d))
        self.register_buffer("_static_length_bin", self._length_bin(length))
    ...
    def _static_terms(self, peak_ids: torch.Tensor) -> torch.Tensor:
        if not hasattr(self, "_static_chrom"):
            raise RuntimeError("Static cache missing; call build_static_cache(coord_table) first.")
        return (
            self.chrom_emb(self._static_chrom[peak_ids])
            + self._static_pos_sin[peak_ids]
            + self.length_emb(self._static_length_bin[peak_ids])
            + self.residual(peak_ids)
        )

    def _value_term(self, value: torch.Tensor) -> torch.Tensor:
        if self.value_as_bias:
            return self.value_bias * value.unsqueeze(-1)
        return self.value_emb(value)

    def embed_ids(self, peak_ids: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
        """Embed peak ids using the static cache (no per-step coord gather)."""
        if self.binarize_values:
            value = (value > 0).to(value.dtype)
        if not hasattr(self, "_static_chrom"):
            raise RuntimeError("Static cache missing; call build_static_cache(coord_table) first.")
        return self._static_terms(peak_ids) + self._value_term(value)

    def forward(self, peak_ids, chrom, pos, length, value) -> torch.Tensor:
        if self.binarize_values:
            value = (value > 0).to(value.dtype)
        if hasattr(self, "_static_chrom"):
            return self._static_terms(peak_ids) + self._value_term(value)
        return (
