"""Train MAMBAVI on the pilot mosaic dataset and write UMAP + mixing metrics.

Run from anywhere::

    python MultiVI/tests/mambavi_pilot_baseline/mambavi_baseline.py

Outputs are written next to this script (``mambavi_pilot_latent.npz``,
``mambavi_pilot_umap.png``, ``mambavi_pilot_metrics.json``, ``model/``).
"""
...
def main() -> None:
    ...
    scvi.model.MAMBAVI.setup_mudata(
        mdata,
        batch_key=BATCH_KEY,
    ...
    model = scvi.model.MAMBAVI(
        mdata,
        mamba3_kwargs={"is_mimo": False},  # MIMO backward exceeds GB10 shared-mem limit
    )

    logger.info("Training MAMBAVI (%d epochs, batch_key=%s)", MAX_EPOCHS, BATCH_KEY)
    ...
    np.savez_compressed(
        OUT_DIR / "mambavi_pilot_latent.npz",
        latent=latent,
        data_type=data_type,
        obs_names=np.asarray(mdata.obs.index, dtype=object),
        umap=emb,
    )
    _plot_umap(emb, data_type, OUT_DIR / "mambavi_pilot_umap.png", n_latent=latent.shape[1])
    (OUT_DIR / "mambavi_pilot_metrics.json").write_text(json.dumps(metrics, indent=2))
